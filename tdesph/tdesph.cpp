  ...
  AthenaArray<Real> &u, AthenaArray<Real> &u_scalar)
{
  ...
  int const js = mb->js, je = mb->je;
  int const ks = mb->ks, ke = mb->ke;
  int const il = is-(ie>is)*ng, iu = ie+(ie>is)*ng;
  int const jl = js-(je>js)*ng, ju = je+(je>js)*ng;
  int const kl = ks-(ke>ks)*ng, ku = ke+(ke>ks)*ng;
  ...
  for (int k = kl; k <= ku; ++k)
    for (int j = jl; j <= ju; ++j)
      for (int i = il; i <= iu; ++i)
      {
        ...
        Real const m1 = u(IVX, k, j, i);
        Real const m2 = u(IVY, k, j, i);
        Real const m3 = u(IVZ, k, j, i);
        ...
        if (NON_BAROTROPIC_EOS)
          u(IEN, k, j, i) +=
            (m1+dm1/2)*dv1 + (m2+dm2/2)*dv2 + (m3+dm3/2)*dv3;
      }
}
  ...
  for (int k = kl; k <= ku; ++k)
    for (int j = jl; j <= ju; ++j)
      for (int i = il; i <= iu; ++i)
      {
        ...
        // magnetic field cannot be added: face-centered magnetic field at half
